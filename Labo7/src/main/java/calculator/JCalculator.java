...
public class JCalculator extends JFrame {
    ...
    private final JTextField jNumber = new JTextField("0.0");
    ...
    private void update() {
        ...
        if(!state.stack().isEmpty()){
            ...
            Double[] stack = state.stack().getData();
            String[] stackString = new String[stack.length];

            for(int i = 0; i < stack.length; i++){
                stackString[i] = stack[i].toString();
            }

            jStack.setListData(stackString);
        } else {
    ...
    public JCalculator() {
        ...
        addOperatorButton("+/-", 1, 5, Color.BLUE, new Negator(state));
        ...
        addOperatorButton("/", 3, 2, Color.RED, new Divider(state));
        addOperatorButton("*", 3, 3, Color.RED, new Multiplier(state));
        addOperatorButton("-", 3, 4, Color.RED, new Subtractor(state));
        addOperatorButton("+", 3, 5, Color.RED, new Adder(state));
